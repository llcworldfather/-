...
export const Deck: React.FC<DeckProps> = ({ stage, onDraw, drawnIndices = [] }) => {
    ...
    return (
        <div className="relative w-full h-[500px] flex items-center justify-center perspective-1000">
            ...
            {stage === 'drawing' && (
                <>
                    {/* Desktop: fan/hover layout; Mobile: horizontal scroll */}
                    <div
                        className="hidden md:flex justify-center transition-all duration-500 px-4 -space-x-48 hover:-space-x-32"
                ...
                /* Mobile: horizontal scrollable layout */
                    <div className="md:hidden w-full overflow-x-auto py-4 px-2">
                        <div className="flex gap-3 px-4" style={{ width: 'max-content' }}>
                            {[...Array(22)].map((_, i) => (
                                <motion.div
                                    key={i}
                                    className={`w-20 h-32 rounded-xl shadow-2xl cursor-pointer active:scale-110 active:shadow-[0_0_30px_rgba(168,85,247,0.6)] transition-all duration-300 relative group overflow-hidden border-2 border-yellow-600/50 bg-slate-900 flex-shrink-0 ${(drawnIndices.includes(i) || drawnIndices.length >= 3) ? 'pointer-events-none opacity-0' : ''}`}
                                    initial={{ opacity: 0, y: 30 }}
                                    animate={{
                                        y: drawnIndices.includes(i) ? -300 : 0,
                                        opacity: drawnIndices.includes(i) ? 0 : 1,
                                        scale: drawnIndices.includes(i) ? 0.3 : 1,
                                    }}
                                    transition={{
                                        duration: drawnIndices.includes(i) ? 1.5 : 0.4,
                                        ease: drawnIndices.includes(i) ? [0.4, 0, 0.2, 1] : 'easeInOut',
                                        delay: drawnIndices.includes(i) ? 0 : i * 0.02
                                    }}
                                    onClick={() => handleCardClick(i)}
                                ...
                                >
                                    ...
                                    {drawnIndices.includes(i) && (
                                        <motion.div
                                            ...
                                            style={{ boxShadow: '0 0 60px rgba(251,191,36,0.9)' }}
                                        />
                                    )}
                                    {/* Card Back Design */}
                                    <div className="absolute inset-1 border border-yellow-600/30 rounded-lg flex items-center justify-center bg-[#1a0b2e] overflow-hidden">
                                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-purple-900/40 via-transparent to-transparent opacity-50"></div>
                                        <div className="w-full h-full opacity-30 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]"></div>
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <div className="w-10 h-10 border-2 border-yellow-600/40 rotate-45 flex items-center justify-center transition-transform duration-700 group-active:rotate-180">
                                                <div className="w-6 h-6 border border-yellow-600/30 rotate-45"></div>
                                            </div>
                                        </div>
                                        <div className="absolute top-1 bottom-1 left-1/2 w-px bg-gradient-to-b from-transparent via-yellow-600/20 to-transparent"></div>
                                        <div className="absolute left-1 right-1 top-1/2 h-px bg-gradient-to-r from-transparent via-yellow-600/20 to-transparent"></div>
                                    </div>
                                    <div className="absolute inset-0 bg-purple-500/0 group-active:bg-purple-500/10 transition-colors duration-300"></div>
                                </motion.div>
                            ))}
                        </div>
                    </div>
